...
export const contact = async(req,res) => {
    try {
        ...
        const usermessage = `Hey, my name is ${name}. 
                             My email is ${email}. My message is ${message}.`;
...
export const addyoutube = async(req,res) => {
    try {
        ...
        res.status(200).json({
            success:true,
            message: "Added to the youtube videos",
        });
    } catch (error) {
        return res.status(400).json({
            success: false,
...
export const deleteyoutube = async(req,res) => {
    try {
        ...
        user.youtube = user.youtube.filter((item) => video._id != id);     
        await user.save();
        res.status(200).json({
            success:true,
